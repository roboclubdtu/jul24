...
namespace choreographer
{
  ...
  struct ResourceContainer
  {
     ...
     */
    [[nodiscard]] const std::string& get_current_stack_name() const
    {
      return current;
    }
     ...
     */
    [[nodiscard]] unsigned int get_current_stack_size()
    {
      return stacks[current].size();
    }
    ...
    */
    ...
    T& get(const size_t index)
    {
      if (current_stack == nullptr)
        throw std::runtime_error("Trying to read current stack of value nullptr");
    ...
    T& get_next()
    {
      if (current_stack == nullptr)
        throw std::runtime_error("Trying to read current stack of value nullptr");
      if (current_stack->size() <= ++current_index)
        throw std::runtime_error("Reached the end of the stack");
